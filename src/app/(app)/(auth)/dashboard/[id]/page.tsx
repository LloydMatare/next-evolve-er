  ...
  const checkPaymentStatus = useCallback(async (orderId: string) => {
   ...
   useEffect(() => {
     const loadOrder = async () => {
       ...
       try {
         // Fetch registration by orderId from the API
         const res = await fetch(`/api/registrations?orderId=${orderIdFromParams}`)
         ...
         if (regData.success && regData.docs?.length > 0) {
           ...
           if (reg.paymentMethod === 'paynow' && reg.status === 'pending') {
             checkPaymentStatus(orderIdFromParams)
           }
         } else {
           setError(true)
         }
     ...
     loadOrder()
   }, [orderIdFromParams, checkPaymentStatus])
        ...
        <section className="relative px-4 pb-10 pt-32 sm:px-6 lg:px-8">
          <div className="container-custom">
            <div className="event-panel-dark rounded-[2.2rem] p-8 md:p-12 text-center">
              ...
              <h1 className="text-3xl font-semibold text-white mb-4">Registration Not Found</h1>
              <p className="text-slate-300 mb-8">
                We couldn't find a registration with that ID. Please check the link or register for the summit.
              </p>
              <Link
                href="/register"
